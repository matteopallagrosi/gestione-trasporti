...
void get_datetime(MYSQL_TIME datetime, char *output) {
		
	sprintf(output, "%04d-%02d-%02d %02d:%02d:%02d", datetime.year, datetime.month, datetime.day, datetime.hour, datetime.minute, datetime.second);
}

void get_date(MYSQL_TIME date, char *output) {
	
	sprintf(output, "%04d-%02d-%02d", date.year, date.month, date.day);
}

void set_date(MYSQL_TIME *date, char *input) {  //get date as YYYY-MM-DD
	char year[5];
	char month[3];
	char day[3];
	...
	memset(date, 0, sizeof(MYSQL_TIME));
...
void set_time(MYSQL_TIME *time, char *input) {  //get time as hh:mm
	char hour[3];
	char minute[3];
	
	memcpy(hour, input, 2);
	hour[2] = '\0';
	memcpy(minute, input+3, 2);
	minute[2] = '\0';
...
void setup_binding(MYSQL *conn, MYSQL_STMT *stmt, MYSQL_BIND **bind){
	unsigned long num_param = mysql_stmt_param_count(stmt);
	
	*bind = malloc(num_param * sizeof(MYSQL_BIND));
	if (*bind == NULL) {
		finish_with_stmt_error(conn, stmt, "Cannot allocate output buffers\n", true);
	}
	
	memset(*bind, 0, num_param * sizeof (MYSQL_BIND));
}
...
//stampa l'intestazione del result set (nomi colonne)
static void dump_result_set_header(MYSQL_RES *res_set)
{
	...
	for (i = 0; i < mysql_num_fields (res_set); i++) {
		field = mysql_fetch_field (res_set);
		col_len = strlen(field->name); //recupero la lunghezza del nome dell' i-esima colonna del result set
		...
		if (field->type == MYSQL_TYPE_TIME) {
			field->max_length = TIME_LENGTH;
		}
		if (field->type == MYSQL_TYPE_DATETIME) {
			field->max_length = DATETIME_LENGTH;
		}
...
//visualizza graficamente il result set (gestisce anche i result set contenenti parametri di output)
void get_result_set(MYSQL *conn, MYSQL_STMT *stmt, char *title, MYSQL_BIND **rs_bind)
{
	int i;
	int status;
	...
	if (num_fields > 0) {
		...
		*rs_bind = (MYSQL_BIND *)malloc(sizeof (MYSQL_BIND) * num_fields);
		...
		for (i = 0; i < num_fields; ++i) { //esegue il bind dell'i-esima colonna con un opportuno buffer allocato dinamicamente
			...
			switch(fields[i].type) {
				...
				case MYSQL_TYPE_LONGLONG:
					...
					break;
				default:
					attr_size = fields[i].max_length; //altrimenti la dimensione del buffer sarà la dimensione del più grande valore su quella colonna
					break;
			}
		...
		if(mysql_stmt_bind_result(stmt, *rs_bind)) {
		 ...
		 if(conn->server_status & SERVER_PS_OUT_PARAMS) {  //il result set corrente è quello contenente i parametri di output (unica riga)
			 if (mysql_stmt_fetch(stmt))
				finish_with_stmt_error(conn, stmt, "Unable to fetch output parameters\n", true);
		 }
		 
		 else {  //si tratta di un result set da visualizzare
			...
			while (true) {
				status = mysql_stmt_fetch(stmt);  //mette nei buffer associati con il bind i valori delle colonne della riga successiva
				...
				for (i = 0; i < num_fields; i++) {
					...
					switch ((*rs_bind)[i].buffer_type) {
						...
						case MYSQL_TYPE_VAR_STRING:
							...
							break;
						case MYSQL_TYPE_DATETIME:
							datetime = (MYSQL_TIME *)(*rs_bind)[i].buffer;
							printf(" %d-%02d-%02d %02d:%02d:%02d |", datetime->year, datetime->month, datetime->day, datetime->hour, datetime->minute, datetime->second);
							break;
						case MYSQL_TYPE_DATE:
						case MYSQL_TYPE_TIMESTAMP:
						...
						case MYSQL_TYPE_TINY:
							...
							break;
						case MYSQL_TYPE_SHORT:
							printf(" %-*.04d |", (int)fields[i].max_length, *(short int *)(*rs_bind)[i].buffer);
							break;
